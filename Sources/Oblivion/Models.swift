...
struct TranscriptSegment: Codable, Identifiable, Equatable {
    ...
    var timestamp: String {
        if source == "import-untimed" { return "Untimed" }
...
struct CallRecord: Codable, Identifiable, Equatable {
    ...
    var generatedNotesSource: String?
    var suggestedNotesSource: String?
    ...
    var preparationText: String {
        ...
        let approved = stories.filter(\.approved).map { "CUE CARD ID \($0.id)\nWHEN TO USE: \($0.trigger)\nEXACT WORDING:\n\($0.body)" }.joined(separator: "\n\n")
...
enum Prompts {
    static let outputStyle = "In user-facing prose, avoid em dashes and colons. Use short sentences, commas, headings, or line breaks. Preserve user-approved cue-card wording, direct quotations, URLs, timestamps, code, and technical syntax exactly. Always use valid JSON when a schema requires it."
    static let personalContext = "Using what you know about me from our conversations and anything I provide, write a concise first-person reference for murmur, my AI meeting copilot. Cover my background, roles, technical and business skills, strongest projects and accomplishments, current goals, and concrete stories useful in interviews, sales calls, and business meetings. Include my specific contributions and confirmed results or metrics. Use factual details to make me sound credible and impressive. Do not invent anything. Ask about important gaps before writing. Use clear headings and short paragraphs without em dashes or colons."
